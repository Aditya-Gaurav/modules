...
angular.module('reports')
  .controller('ReportsController', ['$scope', '$stateParams', '$http', 'Authentication', 'Tenants', 'Reports', 'OntapStorageunits', 'PER_PAGE', 'Flash', '$sanitize',
  function ($scope, $stateParams, $http, Authentication, Tenants, Reports, Storageunits, PER_PAGE, Flash, $sanitize) {
    ...
    $scope.searchByFilter = function(isValid){
      $scope.error = null;
      if (!isValid) {
        $scope.$broadcast('show-errors-check-validity', 'filterReportForm');
        return false;
      }
      var queryParams = {};
      queryParams.filterBy = this.filter;
      Storageunits.getProvisioned(queryParams, function (data) {
        $scope.reports = data;
        $scope.buildPager();
      }, function (errorResponse) {
        throwFlashErrorMessage(errorResponse.data.message);

...
var path = require('path'),
  ...
  InfiniboxSystem = mongoose.model('infinibox_system_health'),
  InfiniboxPool = mongoose.model('infinibox_pool_health'),
  InfiniboxInterface = mongoose.model('infinibox_interface_health'),
  InfiniboxDrive = mongoose.model('infinibox_drive_health'),
  InfiniboxVolume = mongoose.model('infinibox_volume_health'),
...
exports.getOntapHealthData =function(req, res) {
  ...
  //disks name changed into drives
  if (type == "drives") {
...
exports.getInfinidatHealthData =function(req, res) {
  var type = req.params.type;
  console.log("type get  called", type);
  ...
  if (type == "drives") {   
    InfiniboxDrive.find().exec(callback);
  } 

  if (type == "interfaces") {
    InfiniboxInterface.find().exec(callback);    
  } 

  if (type == "pools") {   
    InfiniboxPool.find().exec(callback);
  } 

  if (type == "systems") {
    InfiniboxSystem.find().exec(callback);    
  } 

  if (type == "volumes") {
    InfiniboxVolume.find().exec(callback);    
  } 

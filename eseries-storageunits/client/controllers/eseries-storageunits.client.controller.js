...
angular.module('eseries-storageunits')
	.controller('EseriesStorageunitsController', ['$scope', '$stateParams', '$location', '$http', 'Authentication', 'EseriesStorageunits', 'OntapServers', 'Systems', 'Flash', 'modalService', '$sanitize', 'Tags', 'Applications',
  function ($scope, $stateParams, $location, $http, Authentication, EseriesStorageunits, Servers, Systems, Flash, modalService, $sanitize, Tags, Applications) {
    ...
    $scope.validTargetsToAssign = ["host","group"] //"group" 
      ...
      if (newVal) {
        angular.forEach($scope.systems, function(systemDetail) {
          if (systemDetail.systemId && systemDetail.systemId === newVal) {
            ...
            $scope.existingGroups = systemDetail.groups;
            if ($scope.existingGroups.length == 1) {
              $scope.existingGroup = $scope.existingGroups[0].name
            }
    ...
    $scope.initialize = function() {
      ...
      if(this.mapping == "new") {
        ...
        if(this.portName != undefined) {
          storageunit.portName = ($sanitize(this.portName)).split(','); 
        }
      }

      if(this.mapping == "existing") {
        storageunit.targetType = $sanitize(this.targetType),
        storageunit.hostName =  (this.targetName == 'host') ? $sanitize(this.existingHost) : $sanitize(this.existingGroup)
      }
    ...
    $scope.update = function (isValid) {
      ...
      storageunit.$update({
        ...
        $location.path('eseries-storageunits');
        Flash.create('success', '<strong ng-non-bindable>Submitted the Eseries Storageunit volume expansion Update request.<br>Please wait for the Status to return to Operational.</strong>', 10000, { class: '', id: '' }, true);
      }, function (errorResponse) {
        throwFlashErrorMessage(errorResponse.data.message);
      });
    };

 ...
 * Storage unit Schema
 */
var EseriesStorageunitSchema = new Schema({
  name: {
    type: String,
    default: '',
    trim: true,
 ...
 */
EseriesStorageunitSchema.pre('save', function (next, done) {
  if (!this.isNew) {
    next();
  } else {
    var self = this;   
 ...
 */
EseriesStorageunitSchema.pre('validate', function (next, done) {
  var self = this;
  next();
...
EseriesStorageunitSchema.index({ name: 1, eseriesSystem: 1 }, { unique: true });
EseriesStorageunitSchema.index({ code: 1, eseriesSystem: 1 }, { unique: true });

EseriesStorageunitSchema.methods.toJSON = function() {

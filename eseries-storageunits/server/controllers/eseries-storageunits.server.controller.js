...
exports.create = function (req, res) {
  ...
  function createSu() {
    ...
    var args = {
      ...
      db_info: [{"name":"eseries_storageunits", "id": storageunit._id}, {"name":"jobs", "id":suCreateJob._id}], 
      // action: "create",
    };
    ...
    console.log('EseriesStorageunit create Args: ' , args );
...
exports.update = function (req, res) {
  ...
  if(req.body.fromFix && _.includes(req.user.roles, 'root')){
    ...
    Job.create(req, 'eseries-storageunits', function(err, updateJobRes) {
  ...
  if (!_.isUndefined(req.body.sizegb) && storageunit.sizegb !== req.body.sizegb) {
    ...
    suUpdateJob = updateJobRes;
      storageunit.save(function (err, storageunitSaved) {
        if (err) {
          logger.info('Eseries-storageunit Volume Expansion: Failed to save Eseries-storageunit object: ' + err);
          suUpdateJob.update('Failed', 'Eseries-storageunit Volume Expansion - Failed' , storageunit);
          var errMsg = {};
          _.forOwn(err.errors, function(error, field) {
            logger.info(field, error.message);
            errMsg[field] = error.message;

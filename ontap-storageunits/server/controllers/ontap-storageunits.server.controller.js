...
var _ = require('lodash'),
  ...
  OntapStorageunit = mongoose.model('Storageunit'),
...
exports.create = function (req, res) {
  var clientWfa = require('./ontap-storageunits.server.wfa.su.create');
  var suCreateJob;

	var storageunit = new OntapStorageunit();
  ...
  storageunit.validate(function(err) {
    if (err) {
      ...
      Job.create(req, 'ontap-storageunits', function(err, createJobRes) {
  ...
  function createSu() {
    ...
    console.log('Storage unit create Args:', args);
...
exports.update = function (req, res) {
  ...
  var clientWfa = require('./ontap-storageunits.server.wfa.su.update');
  ...
  if(req.body.fromFix && _.includes(req.user.roles, 'root')){
    ...
    Job.create(req, 'ontap-storageunit', function(err, updateJobRes) {
...
exports.delete = function (req, res) {
  var clientWfa = require('./ontap-storageunits.server.wfa.su.delete.js');
  var storageunit = req.storageunit;
  var suDeleteJob;
...
exports.list = function (req, res) {
  ...
  var query = OntapStorageunit.find({})
    .populate('server', 'name code')
...
exports.storageunitByID = function (req, res, next, id) {
  ...
  console.log("storageunitByID", id)

  OntapStorageunit.findById(id)
  .populate('cluster','name management_ip')
  .populate('server','name code')
  .populate('application', 'name code')
...
exports.getListOfIgroups = function(req, res) {
  ...
  var dbWfa = require('./ontap-storageunits.server.wfa.db.read');
...
exports.getProvisioned = async function(req, res) {
  var filterType = req.query.filterBy;
  var result = [];
    ...
    switch (filterType) {
        ...
        result = await User.aggregate([{
          "$lookup": {
              ...
              "pipeline": [
                ...
                {"$group": {"_id": "$$username", "total": {"$sum": "$sizegb"}, "count": {"$sum": 1}}},
                {"$project": {"username": 1, "total": 1, "count": 1}},
                {"$sort":{"total":1}}
      ...
      case 'application':
        result = await Application.aggregate([{ 
          "$lookup": {
            "from": "storageunits",
            "let":  {"appId": "$_id", "appName": "$name"},
            "pipeline": [
              {"$match": {"$expr": { "$eq": ["$application", "$$appId"]}}},
              {"$group": {"_id": "$$appName", "total": {"$sum": "$sizegb"}, "count": {"$sum": 1}}},
              {"$project": {"appName": 1, "total": 1, "count": 1}},
              {"$sort":{"total":1}}
            ],
            "as": "provisioned",
          },
      ...
      default:
        return respondError(res, 400, 'No filterBy with that identifier has been found');

...
angular.module('ontap-storageunits')
	.controller('OntapStorageunitsController', ['$scope', '$stateParams', '$location', '$http', 'Authentication', 'OntapStorageunits', 'OntapServers', 'Clusters', 'Flash', 'modalService', '$sanitize', 'Tags', 'Applications',
  function ($scope, $stateParams, $location, $http, Authentication, OntapStorageunits, Servers, Clusters, Flash, modalService, $sanitize, Tags, Applications) {
    ...
    $scope.populateIgroups = function(server) {
      $scope.igroups = [];
      var igroups = OntapStorageunits.getIgroups({"vserverName": $scope.serverName, "clusterName": $scope.clusterName});
    ...
    $scope.checkboxChanged = function() {
      if ($scope.dr_enabled && $scope.serverName != "" && $scope.clusterName != "") {
        ...
        var peers = OntapStorageunits.getPeers({"vserverName":$scope.serverName, "clusterName":$scope.clusterName});
    ...
    $scope.initUpdate = function(acl) {
      OntapStorageunits.get({
        ontapStorageunitId: $stateParams.ontapStorageunitId
      ...
      storageunit.$create(function (response) {
        $location.path('ontap-storageunits');
    ...
    $scope.findOne = function () {
      ...
      OntapStorageunits.get({
        ...
        $location.path('ontap-storageunits');
        throwFlashErrorMessage('No ontap-storage unit with that identifier has been found');
      });

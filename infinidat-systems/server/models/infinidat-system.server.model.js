...
var InfinidatSystemSchema = new Schema({
  name: {
    type: String,
    default: '',
    trim: true,
  ...
  rest_url_ip: {
    ...
    match: [ /^([1-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\.([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\.([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])\.([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])$/ , 'REST URL must be in valid format'],
  },
  provisioning_state: {
    type: String,
    default: 'closed',
...
InfinidatSystemSchema.pre('save', function (next) {
  var self = this;
  // self.apis_system_key = self.uuid + ":type=system,uuid="+ self.uuid;
  next();
});

InfinidatSystemSchema.methods.toJSON = function() {
  var obj = this.toObject();
  obj.infinidatSystemId = obj._id;
  delete obj.user;
  delete obj.created;
  delete obj._id;
  delete obj.__v;
...
InfinidatSystemSchema.statics.findByName = function (name, callback) {
  return this.find({ name: name }, callback);
};

mongoose.model('infinidat_systems', InfinidatSystemSchema);
